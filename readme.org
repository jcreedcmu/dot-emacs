...
** Native compilation
Some modes have a lot of warnings with native compilation
#+BEGIN_SRC emacs-lisp
...
(ifat hales (setq custom-file "/tmp/nope.el"))
#+END_SRC
...
#+END_SRC
** Scroll acceleration
I think default mouse wheel acceleration started around emacs 28 or so. I don't think I care for it.
#+BEGIN_SRC emacs-lisp
(setq mouse-wheel-progressive-speed nil)
(setq mouse-wheel-scroll-amount '(3 ((shift) . nil)))
#+END_SRC
...
(defun jcreed-latex-mode-hook ()
  (custom-set-faces
	'(tex-verbatim ((t (:foreground "#999" :background "#103b46")))))
  (set-face-attribute 'tex-verbatim nil  :extend t)
...
(ifat system76
		(setenv "TEXINPUTS" ".:/home/jcreed/dot-emacs/tex//:"))
#+END_SRC
...
(ifat system76 (setq jcreed-rust-enabled t))
(ifat hales (setq jcreed-rust-enabled t))

(when jcreed-rust-enabled
...
(ifat system76
      ...
      (setq exec-path (append exec-path '("/home/jcreed/.elan/bin")))
      (setq lean-rootdir "/home/jcreed/.elan")
      (setq has-lean t))

(ifat hales
		(require 'warnings) ;; eliminate "void-variable warning-suppress-log-types" warning
		(require 'highlight-indent-guides)
		(require 'company)
      (setq load-path (cons "/Users/jcreed/pgit/lean4-mode" load-path))
      (setq exec-path (append exec-path '("/Users/jcreed/.elan/bin")))
      (setq lean-rootdir "/Users/jcreed/.elan")
      (setq has-lean t))

(when (boundp 'has-lean)
  (setq lean-memory-limit 8000)
  (require 'lean4-mode)
  (defun jcreed-lean4-mode-hook ()
    ;; This one way to prevent spurious "Unknown request method: workspace/inlayHint/refresh"
    ...
    (set-face-attribute 'highlight-indent-guides-character-face nil :foreground "#364680")
    (company-mode)
    (setq lsp-enable-file-watchers nil)
    (setq lsp-ui-doc-show-with-mouse nil)
    (define-key lean4-mode-map (kbd "M-;") 'company-complete)
    (define-key lean4-mode-map (kbd "M-.") 'lsp-find-definition)
    (define-key lean4-mode-map (kbd "C-c C-r") 'lsp-find-references)
    (define-key lean4-mode-map (kbd "C-c C-s") 'lsp-rename)
    (define-key lean4-mode-map (kbd "C-c s") 'jcreed-swap-agda-implicit)
    (define-key lean4-mode-map (kbd "C-c C-,") 'lsp-execute-code-action)
...
* Customization for ~hales~

~hales~ is a 2025 MacBook Pro.
...
(ifat system76
      (global-text-scale-adjust 15))

;; Bigger size for retina
(ifat hales
