...
(custom-set-faces
 ...
 '(eglot-highlight-symbol-face ((t (:foreground "#ffff00" :background "#004b56"))))
...
(defun jcreed-typescript-mode-hook ()
  (company-mode)
  (eglot-ensure)
  (setq-local tab-width 2)
  (add-hook 'before-save-hook 'eglot-format)
  (define-key typescript-mode-map (kbd "C-x `") 'flymake-goto-next-error)
  (define-key typescript-mode-map (kbd "C-c C-r") 'xref-find-references)
  (define-key typescript-mode-map (kbd "C-c C-s") 'eglot-rename)
  (define-key typescript-mode-map (kbd "C-c q") 'eglot-code-action-quickfix)
  (define-key typescript-mode-map (kbd "M-.") 'xref-find-definitions)
  (define-key typescript-mode-map (kbd "M-;") 'company-complete))

(add-hook 'typescript-mode-hook 'jcreed-typescript-mode-hook)
#+END_SRC
