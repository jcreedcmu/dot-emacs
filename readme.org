...
*Current status*: I have just divided my config into some sections,
but the work of coalescing them and organizing them better is ongoing.
...
(defvar jcreed-emacs-config-file-name (expand-file-name "~/dot-emacs/readme.org"))
#+END_SRC
...
Define keybindings for notes mode. XXX These are =global-map= and not =notes-mode-map=.
#+BEGIN_SRC emacs-lisp
